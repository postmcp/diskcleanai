...
app reads the newest release on GitHub (`postmcp/diskcleanai`) and installs its zip. New users
download the same release's drag-to-Applications `.dmg` from the website.
...
```
 ...
 scripts/build-release.sh               release v1.2.0                          app checks once a day
   (archive, sign, notarize,     ───▶    ├ DiskCleanAI.zip        ───────────▶ GET api.github.com/repos/postmcp/
    zip + dmg)                              └ DiskCleanAI.dmg  (website download)
...
| --- | --- | --- |
| **The app (zip)** | An asset named `DiskCleanAI.zip` on a GitHub release of `postmcp/diskcleanai` | The in-app updater |
| **The installer (dmg)** | An asset named `DiskCleanAI.dmg` on the same release: the same app in a drag-to-Applications window, itself signed, notarized and stapled | The website's and README's Download button, a permanent link to `github.com/postmcp/diskcleanai/releases/latest/download/DiskCleanAI.dmg` |
...
**c) Build, sign, notarize, zip, dmg:**
```bash
./scripts/build-release.sh          # TEAM_ID defaults to 4P833G76XL, NOTARY_PROFILE to diskcleanai
```
Produces `build/DiskCleanAI.zip` and `build/DiskCleanAI.dmg`, prints their version, sha256 and size,
and prints the exact `gh release create` command for the next step. The app is one **universal**
binary (Apple silicon + Intel); the script stops if a build comes out single-architecture. The app
and then the dmg are each notarized (a few minutes apiece); the script waits and staples both.

The dmg comes from `scripts/make-dmg.sh` (needs `brew install create-dmg`). Its window background is
`scripts/dmg/background.tiff`, generated by `swift scripts/dmg/make-background.swift`; edit that
script and re-run it to change the artwork. The first run may ask to let Terminal control Finder,
which is how the icon layout is saved into the image. `SKIP_DMG=1` builds only the zip.

First run end to end on 24 Sep 2026 (v1.1.0).

**d) Publish the GitHub release:**
```bash
gh release create v1.2.0 build/DiskCleanAI.zip build/DiskCleanAI.dmg --repo postmcp/diskcleanai \
  --title "Disk Clean AI 1.2.0" --notes-file notes.md
```
The tag must be `v` + `MARKETING_VERSION`. Attach exactly one zip and one dmg and keep their names,
**`DiskCleanAI.zip`** (the updater's download) and **`DiskCleanAI.dmg`** (the website's Download button,
a permanent link to `releases/latest/download/DiskCleanAI.dmg`, which GitHub serves from whichever
release is latest). The version lives in the tag, not the file names.
...
curl -sIL -o /dev/null -w '%{http_code}\n' https://github.com/postmcp/diskcleanai/releases/latest/download/DiskCleanAI.dmg   # 200
```
...
- [ ] `notes.md` written
- [ ] `build-release.sh` finished with both notarizations "Accepted" and a stapled app and dmg
- [ ] `gh release create v<version>` with `DiskCleanAI.zip` and `DiskCleanAI.dmg` attached, not marked pre-release
...
`github.com/postmcp/diskcleanai/releases/latest/download/DiskCleanAI.dmg` (`releaseAsset` in
`landing/lib/site.ts`), so a site deploy never
...
| Website download 404s | The latest release has no `DiskCleanAI.dmg`. Upload one: `gh release upload v1.2.0 build/DiskCleanAI.dmg`. |
...
| Publish | `gh release create v<version> build/DiskCleanAI.zip build/DiskCleanAI.dmg --repo postmcp/diskcleanai --notes-file notes.md` |
| Website download link | `https://github.com/postmcp/diskcleanai/releases/latest/download/DiskCleanAI.dmg` |
